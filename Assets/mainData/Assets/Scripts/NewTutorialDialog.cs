...
public class NewTutorialDialog : GUIDynamicWindow
{
	...
	public static NewTutorialDialog CreateWindow()
	{
		NewTutorialDialog newTutorialDialog = new NewTutorialDialog();
		GUIManager.Instance.ShowDynamicWindow(newTutorialDialog, ModalLevelEnum.None);
		NewTutorialManager.setDlg(newTutorialDialog);
		return newTutorialDialog;
	}
	...
	public override bool InitializeResources(bool reload)
	{
		...
		_speakerIcon.TextureSource = "characters_bundle|nick_fury_avengers_HUD_default";
		Add(_speakerIcon);
	...
	{
		if (evt.type == TutorialEvent.TutorialEventType.HIDE_TEXT)
		{
	...
	{
		AnimateOut();
		AppShell.Instance.EventMgr.Fire(this, new TutorialEvent(TutorialEvent.TutorialEventType.TEXT_HIDDEN));
	}
