...
public class SHSCardGameGadgetFriendsWindow : SHSCardGameGadgetCenterWindowBase
{
	public class SelectedFriendsList : GUISimpleControlWindow
	{
		public class SelectedFriendItem : GUISimpleControlWindow
		{
			...
			public SelectedFriendItem(SHSCardGameGadgetFriendsWindow headWindow)
			{
		...
		public SelectedFriendsList(SHSCardGameGadgetFriendsWindow headWindow)
		{
			GUIImage gUIImage = GUIControl.CreateControlFrameCentered<GUIImage>(new Vector2(669f, 479f), new Vector2(4f, -18f));
			...
			PlayWithAnyone.TextureSource = "cardgamegadget_bundle|cardlauncher_anyone_graphic";
			Add(PlayWithAnyone);
		...
		public void AddFriend(Friend friend)
		{
			...
			{
				((SHSCardGameGadgetFriendsWindow)parent).RemoveFriendClickedOn(SelectedFriends[0]);
			}
	...
	public class FriendItem : SHSSelectionItem<GUISimpleControlWindow>, IComparable<FriendItem>
	{
		...
		private SHSCardGameGadgetFriendsWindow headWindow;

		public bool selected;

		private bool trueFriend;

		public FriendItem(Friend friend, bool trueFriend, SHSCardGameGadgetFriendsWindow headWindow)
		{
			this.friend = friend;
	...
	public static string RIGHT_TEXT_ANYONE = "#cgg_gadget_friends_list_right_info_anyone_text";

	public static string PLAY_WITH_ANYONE = "#gadget_play_with_anyone";

	public SHSCardGameGadgetFriendsWindow(SHSCardGameGadgetWindow mainWindow)
		: base(mainWindow)
	{
		...
		GUIButton gUIButton = GUIControl.CreateControlFrameCentered<GUIButton>(new Vector2(256f, 256f), new Vector2(145f, 234f));
		gUIButton.StyleInfo = new SHSButtonStyleInfo("brawlergadget_bundle|L_okbutton_rectangular");
		gUIButton.IsEnabled = true;
		gUIButton.HitTestType = HitTestTypeEnum.Alpha;
		gUIButton.Click += OkWasClicked;
		GUIButton gUIButton2 = GUIControl.CreateControlFrameCentered<GUIButton>(new Vector2(256f, 256f), new Vector2(-10f, 250f));
		gUIButton2.StyleInfo = new SHSButtonStyleInfo("brawlergadget_bundle|L_brawler_airlock_backbutton");
		gUIButton2.HitTestType = HitTestTypeEnum.Alpha;
		gUIButton2.Click += BackWasClicked;
		GUIButton gUIButton3 = GUIControl.CreateControlFrameCentered<GUIButton>(new Vector2(256f, 256f), new Vector2(296f, 247f));
	...
	{
		string[] selectedFriendsArray = SelectedFriendsWindow.GetSelectedFriendsArray();
		if (selectedFriendsArray.Length > 0)
		{
			mainWindow.LaunchManager.FriendsToPlayWithSubmitted(selectedFriendsArray);
			mainWindow.LaunchManager.StartInviteFriends();
		}
		else
		{
			mainWindow.LaunchManager.EnterPvPQueue();
		}
		GUIManager.Instance.ShowDynamicWindow(new SHSCardGameAirlockWindow(), ModalLevelEnum.Default);
	}
